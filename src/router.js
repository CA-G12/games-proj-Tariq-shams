  ...
  if (endpoint === '/') {
    ...
    fs.readFile(pathFile, (err, data) => {
      if (err) {
        ...
        const values = parsedData.map((x) => x.name);
        const filterResults = values.filter((ele) => ele.toLowerCase().includes(game
          .toLowerCase()));
        res.writeHead(200, { 'Content-Type': 'application/json' });
    ...
    let getData = '';
    https.get(`https://www.freetogame.com/api/games?q=${name}`, (request) => {
